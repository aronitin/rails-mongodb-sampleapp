class ExpensesController < ApplicationController
	...
	def new
		# create a new expense object for the currentlyl logged in user.
	...
	def index
		@expenses = current_user.expenses.includes(:expense_type).page(params[:page]).per(2)
	end
	...
	def approvals
		@expenses = Kaminari.paginate_array(Expense.pending.includes(:expense_type)).page(params[:page]).per(2)
